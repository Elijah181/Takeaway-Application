...
def cart():
    ...
    # this route allows the user to go from the menu page to the cart
    # and does calculations for the total value of prices


@app.route('/menu_to_menu')
def menu_to_menu():
    return render_template("menu.html")
# This makes it so when the logo is clicked on the menu page
# it refreshes so there is consistency across the code
...
def item_to_menu():
    ...
    # removes items from the cart as this route is not meant to take items
    # but return without having them in the cart
...
def cart_to_menu():
    return render_template("menu.html")
    # Takes the user from the cart back to the menu.
    # There is a seperate way to clear cart
...
def checkout():
    ...
    # takes the user from the cart to the checkout
    # and clears all items as they have now been brought
...
def submit_data():
    ...
    # these variables all need to be used across the code for
    # them to work properly so they are globalized
    form_id = request.form.get('form_id')
    # this is used to figure out which form is being accessed
    # as there are two within the code
    if form_id == "form_item":
        # this form is the form which figures out which
        # item the user just brought and how to display it
        received_item_id = request.form.get('item_id')
        # this line of code works to figure out
        # what menu item was just selected
        if received_item_id in item_find_list:
            item_index = item_find_list.index(received_item_id)
            # This index is used here and later to keep track of
            # what item was selected
            line_index = item_index + 1
            line_index = line_index * 4
            # This calculation works to find where the items are
            # found within the word document
            ...
            # these lines of code make it so the correct picture can be
            # accesed on the item.html page
            item_name = linecache.getline("words.md", line_index + 1)
            # this is the name of the selected item
            item_description = linecache.getline("words.md", line_index + 2)
            # this is the the description of the selected item
            item_allergies = linecache.getline("words.md", line_index + 3)
            # this gives any important information about what the item contains
            return render_template("item.html",
                                   items_description_show=item_description,
                                   items_name_show=item_name,
                                   item_picture_show=item_picture,
                                   item_allergies_show=item_allergies)
    elif form_id == "form-amount":
        # this form is the form mainly used for figuring out the
        # amount of a specific item the user ordered
        item_amount = request.form.get("amount", type=int)
        # this gets the item amount
        if item_amount is not None:
            if item_amount <= 0:
                ...
                if item_name in item_dict_name_amount_display:
                    # this is for if the item has already been
                    # selected and the user wants to increase the amount
                    item_dict_name_amount_display[item_name] += item_amount
                    # adds the amount for printing later
                    price_index_list = list(item_dict_name_amount_display)
                    # turns dictionary into list
                    price_index = price_index_list.index(item_name)
                    # find the where the item is in the list
                    price *= item_amount
                    # calculates price
                    ...
                    # this is for if the user is adding a new menu item
                    item_dict_name_amount_display[item_name] = item_amount
